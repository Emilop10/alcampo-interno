// src/app/finance/tecnos-decam/page.tsx
'use client';

import { useEffect, useMemo, useState } from 'react';
import { supabase } from '@/lib/supabase';

// ====== CONSTANTES / TABLA PRINCIPAL ======
const TABLE = 'finance_tecnos_decam_bills';
...
// ====== HELPERS GENERALES (mismos patrones que usamos en cobranza) ======
const mxn = (n: number) =>
...
// Evita desfases de zona horaria al formatear yyyy-mm-dd
...
// normalizamos nombres de proveedor para agrupar TECNOS / DECAM
function normSupplier(s: string | null | undefined) {
  return String(s ?? '').toUpperCase().trim();
}
function isTecnos(s: string | null | undefined) {
  const n = normSupplier(s);
  return n.includes('TECNOS') || n.includes('TECNO'); // tolerante
}
function isDecam(s: string | null | undefined) {
  const n = normSupplier(s);
  return (
    n.includes('DECAM') ||
    n.includes('D CAM') ||
...
export default function TecnosDecamPage() {
  // ===== Mes seleccionado =====
  const [ym, setYm] = useState<string>(toMonthInput(new Date()));
  const { start, end } = useMemo(() => monthRange(ym), [ym]);

  // ===== Datos del mes =====
  // invMonthBills = filas cuya fecha de factura cae en el mes
  // paidMonthBills = filas cuya fecha de pago cae en el mes
  ...
  async function reload() {
    ...
    if (qInv.error) {
      console.error(qInv.error);
      setMsg(
        '❌ Error cargando facturas del mes: ' + qInv.error.message
      );
    } else {
      setInvMonthBills((qInv.data || []) as Bill[]);
    }
    ...
    if (qPaid.error) {
      ...
      setMsg((m) =>
        (m ? m + ' · ' : '') +
        '❌ Error cargando pagados del mes: ' +
        (qPaid.error?.message || '')
      );
    } else {
      setPaidMonthBills((qPaid.data || []) as Bill[]);
    }
  }
  ...
  // importe  = suma de amount de TODAS las facturas del mes (por fecha de factura)
  // pagado   = suma de paid_amount de TODOS los pagos hechos este mes (por fecha de pago)
  // pendiente= para esas facturas del mes, amount - paid_amount (aunque se haya pagado en otro mes)
  const totals = useMemo(() => {
    const importe = invMonthBills.reduce(
      (s, r) => s + (Number(r.amount) || 0),
      0
    );
    const pagado = paidMonthBills.reduce(
      (s, r) => s + (Number(r.paid_amount) || 0),
      0
    );
    const pendiente = invMonthBills.reduce((s, r) => {
  ...
  const pendientes = useMemo(
    () =>
      invMonthBills.filter(
        (b) => (b.amount || 0) > (b.paid_amount || 0)
      ),
    [invMonthBills]
  );
  const pagados = useMemo(
    () =>
      paidMonthBills.filter((b) => (b.paid_amount || 0) > 0),
    [paidMonthBills]
  );

  // ===== Totales segmentados TECNOS / DECAM =====
  // Esto es lo que estamos reflejando también en Cobranza.
  const supplierSums = useMemo(() => {
    const facTecnos = invMonthBills
      .filter((b) => isTecnos(b.supplier))
      .reduce((sum, b) => sum + (Number(b.amount) || 0), 0);

    const facDecam = invMonthBills
      .filter((b) => isDecam(b.supplier))
      .reduce((sum, b) => sum + (Number(b.amount) || 0), 0);

    const pagTecnos = paidMonthBills
      .filter((b) => isTecnos(b.supplier))
      .reduce((sum, b) => sum + (Number(b.paid_amount) || 0), 0);

    const pagDecam = paidMonthBills
      .filter((b) => isDecam(b.supplier))
      .reduce((sum, b) => sum + (Number(b.paid_amount) || 0), 0);

    // Pendiente específico por proveedor dentro del mes:
    const penTecnos = invMonthBills
      .filter((b) => isTecnos(b.supplier))
      .reduce((sum, b) => {
        const amt = Number(b.amount) || 0;
        const paid = Number(b.paid_amount) || 0;
        return sum + Math.max(amt - paid, 0);
      }, 0);

    const penDecam = invMonthBills
      .filter((b) => isDecam(b.supplier))
      .reduce((sum, b) => {
        const amt = Number(b.amount) || 0;
        const paid = Number(b.paid_amount) || 0;
  ...
  async function saveBill() {
    try {
      ...
      const amountParsed = r2(
        f.amountText !== '' ? parseMoneyInput(f.amountText) : f.amount
      );
      const isPaid = !!f.markPaid;
      const chosenPaidDate = isPaid
        ? f.paid_date || toISO(new Date())
        : null;
  ...
  function editRow(b: Bill) {
    ...
    // UX: subimos arriba para editar rápido
  ...
  async function toggleDrive(b: Bill) {
    try {
      const { error } = await supabase
        .from(TABLE)
        .update({ drive_uploaded: !b.drive_uploaded })
        .eq('id', b.id);
      if (error) throw error;
      await reload();
    } catch (e: any) {
      console.error(e);
      setMsg(
        '❌ No se pudo actualizar “Subido a Drive”.'
      );
    }
  }

  // ===== Pagos inline =====
  const [payDateMap, setPayDateMap] = useState<
    Record<string, string>
  >({});
  const [paidDateMap, setPaidDateMap] = useState<
    Record<string, string>
  >({});

  useEffect(() => {
    const today = toISO(new Date());
    const newPay = { ...payDateMap };
    const newPaid = { ...paidDateMap };

    pendientes.forEach((b) => {
      if (!newPay[b.id]) newPay[b.id] = today;
    });

    pagados.forEach((b) => {
      if (!newPaid[b.id])
        newPaid[b.id] = b.paid_date || today;
    });
  ...
  const setRowPayDate = (id: string, v: string) =>
    setPayDateMap((m) => ({ ...m, [id]: v }));
  const setRowPaidDate = (id: string, v: string) =>
    setPaidDateMap((m) => ({ ...m, [id]: v }));

  async function markPaid(b: Bill) {
    try {
      const date =
        payDateMap[b.id] || toISO(new Date());
      const { error } = await supabase
        .from(TABLE)
        .update({
          paid_amount: r2(b.amount),
          paid_date: date,
        })
        .eq('id', b.id);
      if (error) throw error;
  ...
  async function unsetPaid(b: Bill) {
    try {
      ...
      if (error) throw error;
      setMsg(
        '↩️ Pago quitado (vuelto a pendiente).'
      );
      await reload();
    } catch (e: any) {
      console.error(e);
      setMsg(
        '❌ No se pudo quitar el pago.'
      );
    }
  }
  ...
  async function exportToExcel() {
    try {
      ...
      ws0.columns = [
        ...
        { header: 'Monto', key: 'v', width: 18 },
      ];
      ws0.getRow(1).eachCell((c) =>
        Object.assign(c, headerStyle)
      );

      ws0.addRow({
        k: 'Facturado en el mes (fecha fac)',
        v: totals.importe,
      });
      ws0.addRow({
        k: 'Pagado en el mes (fecha pago)',
        v: totals.pagado,
      });
      ws0.addRow({
        k: 'Pendiente del mes',
      ...
      // Totales específicos TECNOS / DECAM
      ws0.addRow({
        k: 'TECNOS — Facturado (mes)',
        v: supplierSums.facTecnos,
      });
      ws0.addRow({
        k: 'TECNOS — Pagado (mes)',
        v: supplierSums.pagTecnos,
      });
      ws0.addRow({
        k: 'TECNOS — Pendiente (mes)',
        v: supplierSums.penTecnos,
      });
      ws0.addRow({
        k: 'DECAM — Facturado (mes)',
        v: supplierSums.facDecam,
      });
      ws0.addRow({
        k: 'DECAM — Pagado (mes)',
        v: supplierSums.pagDecam,
      });
      ws0.addRow({
        k: 'DECAM — Pendiente (mes)',
        v: supplierSums.penDecam,
      });
      ...
      ws1.columns = [
        ...
        { header: 'Fecha pago (plan)', key: 'paid_plan', width: 14 },
      ...
      pendientes.forEach((b) => {
        ...
        ws1.addRow({
          ...
          check_no: b.check_no || '',
          bank: b.bank || '',
          paid_plan: payDateMap[b.id] || toISO(new Date()),
          drive_uploaded: b.drive_uploaded ? 'Sí' : 'No',
          notes: b.notes || '',
        });
      });

      ws1.getColumn('invoice_date').numFmt = dateFmt;
      ws1.getColumn('paid_plan').numFmt = dateFmt;
      ['amount', 'paid_amount', 'saldo'].forEach(
        (k) => (ws1.getColumn(k).numFmt = currencyFmt)
      );
      ...
      ['amount', 'paid_amount'].forEach(
        (k) => (ws2.getColumn(k).numFmt = currencyFmt)
      );
      ...
      a.download = `TecnosDecam_${ym}.xlsx`;
    ...
    <div className="max-w-7xl mx-auto p-6">
      ...
      <div className="mb-4 flex flex-wrap items-end gap-3">
        ...
        <span className="text-xs text-gray-500">
          Rango:{' '}
          {parseYMDLocal(start).toLocaleDateString(
            'es-MX',
            {
              month: 'long',
              year: 'numeric',
        ...
        <div className="ml-auto flex flex-wrap gap-2">
          ...
          <div className="border rounded p-2 text-sm">
            ...
            <div className="font-semibold">
              {mxn(totals.pendiente)}
            </div>
          </div>

          {/* TARJETAS DETALLADAS TECNOS / DECAM */}
          <div className="border rounded p-2 text-sm">
            <div className="text-xs text-gray-500">
              TECNOS (mes)
            </div>
            <div className="font-semibold">
              {mxn(supplierSums.facTecnos)}
            </div>
            <div className="text-[11px] text-gray-500 mt-0.5">
              Pag: {mxn(supplierSums.pagTecnos)} · Pend:{' '}
              {mxn(supplierSums.penTecnos)}
            </div>
          </div>
          ...
          <div className="border rounded p-2 text-sm">
            ...
            <div className="font-semibold">
              {mxn(supplierSums.facDecam)}
            </div>
            <div className="text-[11px] text-gray-500 mt-0.5">
              Pag: {mxn(supplierSums.pagDecam)} · Pend:{' '}
              {mxn(supplierSums.penDecam)}
            </div>
          </div>
